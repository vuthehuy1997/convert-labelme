...
def convertlabelme2icdar(filepath, txt_file):
    ...
    for shape in shapes:
        ...
        annotation = {"transcription": label_text, "points": points}
        annotations.append(annotation)
    json_string = json.dumps(annotations, ensure_ascii=False).encode('utf8')
    json_string = json_string.decode()
    line = f"{os.path.join('images', image_filename)}\t{json_string}\n"
    txt_file.write(line)
...
if __name__ == "__main__":
    ...
    if is_split is True:
        ...
        TRAIN_FILE = io.open(
            os.path.join(ICDAR_DIR, "train.txt"), "w", encoding="utf8"
        )
        TEST_FILE = io.open(
            os.path.join(ICDAR_DIR, "test.txt"), "w", encoding="utf8"
        )
        ...
        ALL_FILE = io.open(
            os.path.join(ICDAR_DIR, "gt.txt"), "w", encoding="utf8"
        )
